...
export class YoutubeAdapter extends PlatformAdapter {
  ...
  async extract(url: string): Promise<VideoMetadata> {
    this.logger.log(`Extracting YouTube URL: ${url}`);
    ...
    try {
      ...
      const downloads: VideoQuality[] = info.formats
        // Filter for streams with both video/audio or just video if audio is separate (handling adaptive later? for now just simple)
        // Actually distube/ytdl-core often separates them. We want 'hasAudio' true for simple downloads.
        .filter((format) => format.container === 'mp4' && format.hasAudio)
        .map((format) => ({
          resolution: format.qualityLabel || 'Unknown',
          url: format.url,
          format: (format.container as any) || 'mp4',
      ...
      this.logger.error(`YouTube extraction failed: ${error.message}`);
